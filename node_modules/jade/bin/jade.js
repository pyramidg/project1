...
program.on('--help', function(){
  ...
  console.log('');
  console.log('    # translate ejs the templates dir');
  console.log('    $ ejs templates');
  console.log('');
  console.log('    # create {foo,bar}.html');
  console.log('    $ ejs {foo,bar}.ejs');
  console.log('');
  console.log('    # ejs over stdio');
  console.log('    $ ejs < my.ejs > my.html');
  console.log('');
  console.log('    # ejs over stdio');
  console.log('    $ echo \'h1 ejs!\' | ejs');
  console.log('');
...
function stdin() {
  ...
  process.stdin.on('end', function(){
    var output;
    if (options.client) {
      output = ejs.compileClient(buf, options);
    } else {
      var fn = ejs.compile(buf, options);
...
function renderFile(path, rootPath) {
  ...
  if (stat.isFile() && re.test(path)) {
    ...
    var fn = options.client
           ? ejs.compileFileClient(path, options)
           : ejs.compileFile(path, options);
    ...
    if (program.out) {
      ...
      if (rootPath && program.hierarchy) {
        ...
        if (rootPath && !hierarchyWarned) {
          console.warn('In ejs 2.0.0 --hierarchy will become the default.');
